...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { key: 'intro', title: '内容简介', editType: 'string' },
        { key: 'courseId', title: '课程id', editType: 'number' },
        { key: 'parentId', title: '上级id', editType: 'number' },
        { key: 'type', title: '类型（章、节）', editType: 'number' },
        { key: 'createTime', title: '创建时间', editType: 'date' },
  ...
  methods: {
    initData (page) {
      ...
      rest.getAll(`sections?page=${page}`).then(res => {
        _this.tableData = res.data._embedded.sections.map(item => {
          item._checked = false
          return item
        })
    ...
    handleSave (row, index, editing) {
      ...
      if (add) {
        // 删除 editing的add标志
        delete editing['add']
        rest.addOne('sections', editing).then(res => {
          _this.tableData.unshift(res.data)
          _this.tableData.pop()
        })
      } else {
        rest.putOne('sections', editing).then(res => {
