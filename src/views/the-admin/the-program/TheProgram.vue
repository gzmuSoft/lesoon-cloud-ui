...
export default {
  ...
  methods: {
    initData (page) {
      ...
      rest.getAll(`programs?page=${page}`).then(res => {
        _this.tableData = res.data._embedded.programs.map(item => {
          item._checked = false
          return item
        })
    ...
    handleSave (row, index, editing) {
      ...
      if (add) {
        delete editing['add']
        rest.addOne('programs', editing).then(res => {
          _this.tableData.unshift(res.data)
          _this.tableData.pop()
        })
      } else {
        rest.putOne('programs', editing).then(res => {
